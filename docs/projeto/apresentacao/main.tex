...
\begin{frame}{Protocolo experimental e rastreabilidade}
  \begin{columns}[T,onlytextwidth]
    \column{.55\textwidth}
      \centering
      {\Large\bfseries 4 cenários $\times$ 1.000 seeds}\par
      \vspace{.3em}
      {\Large\bfseries $\times$ 5 algoritmos = 20.000 execuções}\par
      \vspace{.8em}
      {\large 1.000 processos por carga}\par
      \vspace{.8em}
      Média e intervalo de confiança de 95\%
    \column{.42\textwidth}
      \begin{block}{Rastreabilidade}
        \begin{itemize}
...
\begin{frame}{Resultado 1: turnaround médio}
  ...
  \small \textbf{Leitura:} Prioridade teve o menor turnaround em três cenários e SJF no I/O-bound; RR apresentou o maior, especialmente em CPU-bound. Barras mostram média e IC95\%, em ticks.
\end{frame}
...
\begin{frame}{Resultado 2: justiça de Jain do slowdown}
  ...
  \small \textbf{Leitura:} RR alcança alta justiça, sobretudo em CPU-bound; Prioridade apresenta a maior desigualdade. O eixo vertical está em porcentagem e as hastes representam IC95\%.
\end{frame}
...
\begin{frame}{Convergência com o aumento da amostra}
  \centering
  \figuraresultado{comparativo_convergencia_ic95.png}

  \small \textbf{Leitura:} as médias estabilizam progressivamente e os IC95\% estreitam de $N=10$ a $N=1.000$, aumentando a precisão sem alterar a tendência geral.
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{.55\textwidth}
      \begin{block}{O que os dados mostraram}
        \begin{itemize}
          \item PDBH não apresentou vantagem prática nos cenários avaliados
          \item IC95\% praticamente sobreposto ao FCFS
          \item Mesmo número de trocas de FCFS e Prioridade
    ...
    \column{.42\textwidth}
      ...
      \end{block}
      \alert{Conclusão:} neste modelo, o histórico dinâmico isolado não trouxe ganho. Próximo passo: investigar um PDBH preemptivo.
  \end{columns}
  \vfill
  \centering\small Código testado • resultados consolidados • artigo científico • responsabilidades registradas
\end{frame}
